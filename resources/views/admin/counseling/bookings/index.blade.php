        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border-t-4 border-primary">
            <div class="p-8 text-secondary">
                ...
                <div class="flex justify-between items-center mb-6">
                    <p class="text-gray-600">View and manage counseling session bookings.</p>
                    
                    <!-- Status Filter -->
                    <form method="GET" action="{{ route('admin.counseling.bookings') }}" class="flex items-center gap-2">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Session Date & Time</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($bookings as $booking)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <form action="{{ route('admin.counseling.bookings.status', $booking->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        ...
                                        <div class="text-xs text-gray-400">{{ $booking->created_at->format('g:i A') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        @endif
                                        <form action="{{ route('admin.counseling.bookings.destroy', $booking->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this booking? This will free up the slot.');">
                                            @csrf
                                            @method('DELETE')
