        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border-t-4 border-primary">
            <div class="p-8 text-secondary">
                ...
                <div class="flex justify-between items-center mb-6">
                    <p class="text-gray-600">Manage available time slots for management sessions.</p>
                    <a href="{{ route('admin.management-session.slots.create') }}" class="bg-primary hover:bg-accent text-white font-bold py-2 px-6 rounded-full shadow-md transform transition hover:-translate-y-1 hover:shadow-lg text-sm">
                            ...
                            @forelse($slots as $slot)
                                <tr class="{{ $slot->date < now()->startOfDay() ? 'bg-gray-50 opacity-60' : '' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        @if($slot->bookings)
                                            <div class="text-gray-900">{{ $slot->bookings->name }}</div>
                                            <div class="text-gray-500 text-xs">{{ $slot->bookings->event_type }}</div>
                                        @else
                                            <span class="text-gray-400">-</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('admin.management-session.slots.edit', $slot->id) }}" class="text-primary hover:text-accent mr-3">Edit</a>
                                        @if(!$slot->is_booked)
                                            <form action="{{ route('admin.management-session.slots.destroy', $slot->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this slot?');">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                        ...
                                        <a href="{{ route('admin.management-session.slots.create') }}" class="text-primary hover:underline mt-2 inline-block">Create your first slot</a>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
