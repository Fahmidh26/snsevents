                    ...
                    <div class="bg-blue-50 border-l-4 border-blue-500 text-blue-700 p-4 mb-6 rounded-r">
                        <div class="flex items-start">
                            ...
                            <div>
                                ...
                                <p class="text-sm mt-1">Booked by: {{ $slot->bookings->name }} ({{ $slot->bookings->email }})</p>
                                <p class="text-sm">Status: <span class="font-medium">{{ ucfirst($slot->bookings->status) }}</span></p>
                                <a href="{{ route('admin.management-session.bookings') }}" class="text-sm underline mt-2 inline-block">View all bookings →</a>
                            </div>
                        </div>
                    </div>
                @endif

                <form action="{{ route('admin.management-session.slots.update', $slot->id) }}" method="POST">
                    @csrf
                    @method('PUT')
