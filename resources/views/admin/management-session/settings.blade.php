    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border-t-4 border-primary">
            <div class="p-8 text-secondary">
                ...
                <p class="text-gray-600 mb-6">Configure your management session booking page settings, intro text, and session details.</p>

                <form action="{{ route('admin.management-session.settings.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-6">
                        <label class="flex items-center cursor-pointer">
                            ...
                            <span class="ml-3 text-sm font-medium text-gray-700">Enable Management Session Booking Page</span>
                        </label>
                        <p class="text-xs text-gray-500 mt-1 ml-8">When disabled, the booking page will return a 404 error.</p>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="intro_title" id="intro_title" value="{{ old('intro_title', $settings->intro_title ?? 'Plan Your Event with Experts') }}" 
                            class="w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                        @error('intro_title')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-xs text-gray-500 mt-1">This text will appear at the top of the booking page. You can use line breaks for paragraphs.</p>
                        @error('intro_text')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="email" name="contact_email" id="contact_email" value="{{ old('contact_email', $settings->contact_email) }}" 
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                            <p class="text-xs text-gray-500 mt-1">Displayed on confirmation page.</p>
                            @error('contact_email')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="seo_title" id="seo_title" value="{{ old('seo_title', $settings->seo_title) }}" 
                                placeholder="Management Consultation - SNS Events" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                            @error('seo_title')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="seo_keywords" id="seo_keywords" value="{{ old('seo_keywords', $settings->seo_keywords) }}" 
                                placeholder="event planning, management consultation, event logistics" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                            @error('seo_keywords')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
