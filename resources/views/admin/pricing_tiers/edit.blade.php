<x-admin-layout>
    <x-slot name="header">
        Edit Pricing Tier: {{ $pricingTier->tier_name }}
    </x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-4xl" x-data="{ features: {{ json_encode($pricingTier->features ?? ['']) }} }">
        <form action="{{ route('admin.pricing-tiers.update', $pricingTier) }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="event_type_id" required class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all">
                        ...
                        @foreach($eventTypes as $type)
                            <option value="{{ $type->id }}" {{ old('event_type_id', $pricingTier->event_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Tier Name *</label>
                    <input type="text" name="tier_name" value="{{ old('tier_name', $pricingTier->tier_name) }}" required class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" name="price" value="{{ old('price', $pricingTier->price) }}" required class="w-full pl-8 pr-4 py-2 rounded-lg border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all">
                    </div>
                ...
                <div>
                    ...
                    @if($pricingTier->image)
                        <div class="mb-2">
                            <img src="{{ asset($pricingTier->image) }}" alt="Current image" class="w-32 h-32 object-cover rounded-lg border border-gray-200">
                        </div>
                    @endif
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="2" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all">{{ old('description', $pricingTier->description) }}</textarea>
                </div>
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="status" id="status" {{ old('status', $pricingTier->status) ? 'checked' : '' }} class="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary">
            ...
            <div class="mt-8 flex justify-end">
                <button type="submit" class="bg-primary hover:bg-accent text-white px-8 py-3 rounded-lg font-bold transition-all shadow-lg hover:shadow-xl flex items-center gap-2">
                    <i class="fas fa-save"></i> Update Pricing Tier
                </button>
            </div>
        </form>
    </div>
